...
const makeMessagesSocket = (config) => {
    ...
    const refreshMediaConn = async (forceGet = false) => {
        ...
        const sendRichMessage = async (jid, subMessages, quoted = null, options = {}) => {
            ...
            return await sock.sendMessage(jid, richContent, { 
                quoted, 
                ...options 
            });
        };

        // Helper supaya lebih gampang dipakai
        const sendTable = async (jid, title, rows, quoted = null, options = {}) => {
            return sendRichMessage(jid, [{ messageType: 4, tableMetadata: { title, rows } }], quoted, options);
        };

        const sendCodeBlock = async (jid, code, quoted = null, options = {}) => {
            return sendRichMessage(jid, [{ messageType: 5, code }], quoted, options);
        };

        const sendRichText = async (jid, text, quoted = null, options = {}) => {
            return sendRichMessage(jid, [{ messageType: 2, text }], quoted, options);
        };
