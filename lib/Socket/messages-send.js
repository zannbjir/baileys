...
const makeMessagesSocket = (config) => {
    ...
    const sendRichMessage = async (jid, subMessages, quoted = null, options = {}) => {
        if (!Array.isArray(subMessages) || subMessages.length === 0) {
            throw new Error('subMessages harus array dan tidak boleh kosong!');
        }
                ...
                message: {
                    richResponseMessage: {
                        subMessages: subMessages.map(sub => {
                            if (sub.messageType === 2 || sub.text || sub.messageText) {
                                return { message: { extendedTextMessage: { text: sub.text || sub.messageText || '' }}};
                            }
                            if (sub.messageType === 4 || sub.tableMetadata || sub.table) {
                                const table = sub.tableMetadata || sub.table;
                                return {
                                    message: {
                                        tableMessage: {
                            ...
                            if (sub.messageType === 5 || sub.code || sub.codeMetadata) {
                                const code = sub.code || sub.codeMetadata?.code || sub.text || '';
                                return { message: { extendedTextMessage: { text: `\`\`\`${code}\`\`\`` }}};
                            }
                            return sub;
                        })
                    }
        ...
        return await sock.sendMessage(jid, richContent, { quoted, ...options });
    };

    const sendTable = async (jid, title, rows, quoted = null, options = {}) => 
        sendRichMessage(jid, [{ messageType: 4, tableMetadata: { title, rows }}], quoted, options);

    const sendCodeBlock = async (jid, code, quoted = null, options = {}) => 
        sendRichMessage(jid, [{ messageType: 5, code }], quoted, options);

    const sendRichText = async (jid, text, quoted = null, options = {}) => 
        sendRichMessage(jid, [{ messageType: 2, text }], quoted, options);
